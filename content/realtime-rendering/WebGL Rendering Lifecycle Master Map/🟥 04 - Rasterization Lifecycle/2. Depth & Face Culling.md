...
-   **Z-Buffer (Depth Buffer)**: [[⚫ 08 - Framebuffer → Display Lifecycle/1. Framebuffer 구조|Framebuffer]]의 각 픽셀에 대한 깊이(Z 값) 정보를 저장하는 2D 버퍼입니다. 화면에 가까울수록 작은 값, 멀수록 큰 값을 가집니다 (일반적으로 0.0 ~ 1.0 사이).
