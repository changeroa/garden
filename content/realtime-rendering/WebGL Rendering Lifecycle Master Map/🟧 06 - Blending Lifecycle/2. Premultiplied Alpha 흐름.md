...
-   **문제**: 일반적인 알파 블렌딩 `(Src * A) + (Dst * (1-A))`은 [[🟫 07 - Color & Gamma Lifecycle/1. 선형 vs sRGB|선형(Linear) 색 공간]]에서 수행될 때만 수학적으로 올바릅니다. 하지만 많은 경우 텍스처는 [[🟫 07 - Color & Gamma Lifecycle/1. 선형 vs sRGB|sRGB(감마 보정된) 공간]]에 저장되어 있습니다. 감마 공간에서 이 연산을 하면, 특히 어두운 색상 주변에 검은색 후광(dark fringe)이나 원치 않는 색상 왜곡이 발생합니다.
...
이 흐름을 따르면 [[🧩 09 - Rendering Lifecycle 디버깅 체계/Rendering Lifecycle 디버깅 체계|반투명 경계의 지저분한 문제나 색상 왜곡]]을 크게 줄일 수 있습니다.
