# MCP Request Handler 동작 원리

Created: 2025-12-30 14:15
Modified: 2025-12-30 14:15
TemplateVersion: v1
Status: #status/seedling
Tags: #knowledge #mcp #request-handler #architecture
...
MCP Request Handler 동작 원리는 클라이언트 요청을 받아 도구 이름(name)으로 분기하고, 타입 캐스팅(as unknown as)으로 입력을 변환하여 적절한 핸들러 함수에 전달하는 과정이다.

---

## Core Idea
서버는 호텔 프론트 데스크처럼 요청의 종류(name)를 보고 적절한 담당자(핸들러 함수)에게 연결해주는 분배기 역할을 한다.
MCP SDK는 args를 '무슨 모양인지 모르는 객체'로 전달하므로, 각 도구에 맞는 타입(GetMocInput, InboxNoteInput 등)으로 변환해야 한다.

## Why It Matters
- 요청 처리 흐름을 이해하면 새로운 도구를 추가할 때 switch 문에 case만 추가하면 되므로 확장이 쉽다.

## Explanation
- 배송(요청 수신): Claude가 '지식 노트 만들어줘'라고 요청, args 상자에는 내용물 정보 없음
- 분류(switch 문): 송장(name)을 보고 'create_knowledge_note'임을 확인
- 개봉 및 확인(Type Casting): as unknown as CreateKnowledgeNoteInput으로 상자 내용물 타입 확정
- 작업 수행(Function Call): handleCreateKnowledgeNote에 전달하여 실제 노트 생성
...
## Self-Test
- Q: MCP 서버에서 새로운 도구를 추가하려면 어떤 부분을 수정해야 하는가?
- A: switch(name) 문에 새로운 case를 추가하고, 해당 도구의 핸들러 함수를 호출한다.
